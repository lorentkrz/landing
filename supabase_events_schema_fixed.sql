-- ========================================
-- EVENTS SYSTEM FOR NATA APP (FIXED VERSION)
-- ========================================
...
CREATE TABLE IF NOT EXISTS venue_events (
  ...
  music_genre TEXT[], -- Changed from VARCHAR[] to TEXT[] for PostgreSQL compatibility
...
CREATE TABLE IF NOT EXISTS event_attendees (
  ...
  user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE, -- Fixed: profiles(id) not profiles(user_id)
...
DROP TRIGGER IF EXISTS log_favorite_activity_trigger ON user_favorites;
...
SELECT 
  ...
  v.latitude,
  v.longitude,
  COALESCE(ea.attendee_count, 0) as current_attendees
...
CREATE OR REPLACE VIEW friends_activity_feed AS
SELECT 
  fa.*,
  p.first_name,
  p.last_name,
